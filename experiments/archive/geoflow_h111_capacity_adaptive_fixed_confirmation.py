...
This script preserves an H10.11 GPT-2 LoRA confirmation experiment. It is not
part of the public optimizer API and is not imported by ``geometric_flow``.
The script keeps the experimental GPT-2 Conv1D factor layout used during that
run, which is transposed relative to the library convention ``M = B @ A``.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[1201, 1303, 1409, 1511, 1601, 1709, 1801, 1907, 2003, 2111],
    )
    ...
    parser.add_argument(
        "--local-function-tolerance-grid",
        nargs="+",
        type=float,
        default=[0.05],
        help="Maximum predicted local product-space displacement; K is generated from capacity.",
    )
    ...
    parser.add_argument(
        "--repo-dir",
        default="/content/Geometric-Flow-h111",
    )
    parser.add_argument(
        "--out-dir",
        default="/content/geoflow_h111_results",
    )
    parser.add_argument(
        "--local-model-dir",
        default="/content/hf_models/gpt2_h111",
    )
    ...
    if len(args.local_function_tolerance_grid) != 1:
        raise ValueError(
            "H10.11 is a fixed confirmation. Provide exactly one local tolerance."
        )
    if any(value <= 0 for value in args.local_function_tolerance_grid):
        raise ValueError("The local tolerance must be positive")
    return args
...
def main():
    ...
    config_payload = {
        **asdict(cfg),
        ...
        "local_function_tolerance_grid": list(
            args.local_function_tolerance_grid
        ),
        "fixed_configuration": {
            "macro_flow_time": args.macro_flow_time,
            "local_function_tolerance": args.local_function_tolerance_grid[0],
        },
    ...
    print("=" * 148)
    print("GeoFlow H10.11 -- FIXED CAPACITY-ADAPTIVE CONFIRMATION")
    print("=" * 148)
    ...
    for seed in cfg.seeds:
        ...
        for tolerance in args.local_function_tolerance_grid:
    ...
    for tolerance, group in groups.items():
        ...
        aggregate = {
            "macro_flow_time": args.macro_flow_time,
            "local_function_tolerance": tolerance,
            "n": len(group),
    ...
    if len(aggregates) != 1:
        raise RuntimeError(
            f"H10.11 expected exactly one fixed configuration, got {len(aggregates)}"
        )
    winner = aggregates[0]

    winner_rows = groups[winner["local_function_tolerance"]]
    ...
    gates = {
        "H111_FIXED_CONFIG_ONLY":
            len(args.local_function_tolerance_grid) == 1,
        "H111_HELD_OUT_SEEDS_10":
            len(args.seeds) == 10,
        "H111_AUTO_K_GENERATED":
            winner["mean_auto_substeps"] > 0,
        "H111_K_NOT_PREDETERMINED":
            winner["max_auto_substeps"] > winner["min_auto_substeps"],
        "H111_LOCAL_DPHI_TOLERANCE_RESPECTED":
            all(
                row.max_predicted_local_dphi
                <= row.local_function_tolerance * (1.0 + 1e-6)
                for row in winner_rows
            ),
        "H111_ALL_SEEDS_MATCHED_PROGRESS":
            winner["all_matched_progress"],
        "H111_ALL_SEEDS_GAUGE_BETTER_THAN_ADAM":
            winner["all_gauge_better_than_adam"],
        "H111_MEAN_GAUGE_SUPPRESSION_10X_PASS":
            winner["geometric_mean_gauge_ratio"] <= 0.1,
        "H111_GAUGE_10X_SEED_RATE_80_PERCENT_PASS":
            float(np.mean([
                row.gauge_suppression >= 10.0 for row in winner_rows
            ])) >= 0.8,
        "H111_GAUGE_7X_ALL_SEEDS_PASS":
            all(row.gauge_suppression >= 7.0 for row in winner_rows),
        "H111_P20_SUPPRESSION_10X_PASS":
            winner["p20_gauge_suppression"] >= 10.0,
        "H111_MIN_SUPPRESSION_7X_PASS":
            winner["min_gauge_suppression"] >= 7.0,
        "H111_BOOTSTRAP_UPPER_GAUGE_RATIO_BELOW_0P1_PASS":
            float(ci_high) <= 0.1,
        "H111_NO_FLOW_DT_CAP_HITS":
            winner["total_dt_cap_hits"] == 0,
        "H111_NO_FALLBACK_PASS":
    ...
    print("\n" + "=" * 148)
    print("H10.11 FIXED CAPACITY-ADAPTIVE SUMMARY")
    print("=" * 148)
