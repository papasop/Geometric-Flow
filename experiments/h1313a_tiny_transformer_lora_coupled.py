...
# -*- coding: utf-8 -*-
"""
H13.13A — TINY TRANSFORMER / LoRA COUPLED-COVARIANCE SMOKE AUDIT
...
Typical smoke run
-----------------
python h1313a_tiny_transformer_lora_coupled.py \
  --trials 2 --steps 80 --train-samples 512 --val-samples 128 \
  --batch-size 16 --probe-steps 0,20,40,79 \
  --output-dir h1313a_smoke
...
def parse_args() -> Config:
    ...
    while i < len(unknown):
        if unknown[i] == "-f" and i + 1 < len(unknown):
            ...
            i += 1
    if ignored:
        print(f"[H13.13A] ignored notebook/kernel arguments: {ignored}")
...
def main() -> int:
    ...
    print("=" * 120)
    print("H13.13A TINY TRANSFORMER / LoRA COUPLED-COVARIANCE SMOKE AUDIT")
    print("=" * 120)
    ...
    payload = {
        "title": "H13.13A tiny Transformer/LoRA coupled covariance audit",
        "config": asdict(cfg),
        "summary": summary,
        "gates": gates,
        "limits": [
