...
# -*- coding: utf-8 -*-
"""
H13.11 — GAUGE-COVARIANT MOMENTUM AND SECOND-MOMENT GEOFLOW

Goal
----
Construct a history-aware GeoFlow whose optimizer state lives in executed
functional channels rather than raw factor coordinates.
...
Main questions
--------------
1. Does channel-space momentum preserve gauge covariance?
2. Does channel-space second-moment normalization reduce functional variance?
3. Does history-aware channel GeoFlow outperform plain split GeoFlow under a
   matched product-step budget?
4. Does it retain the H13.9/H13.10 structural guarantees?
...
Smoke test
----------
python h1311_gauge_covariant_momentum.py \
...
def full_product_direction(
    ...
    j = torch.cat(
        [torch.kron(a.T.contiguous(), eye_m), torch.kron(eye_n, b)],
        dim=1,
    )
...
def init_state(b: Tensor, a: Tensor, method: str) -> MethodState:
    ...
    if method in ("channel_momentum_geoflow", "channel_adaptive_geoflow"):
...
def channel_candidate(
    ...
    if update:
        ...
        state.t += 1
        ua, ub, qa, qb, t = state.u_a, state.u_b, state.q_a, state.q_b, state.t
    else:
...
def transformed_probe_state(state: MethodState, method: str, s: Tensor) -> MethodState:
    ...
    elif method in ("channel_momentum_geoflow", "channel_adaptive_geoflow"):
        assert state.channel is not None
        # Channel state is already gauge invariant.
...
def gates(summary: List[Dict[str, object]], cfg: Config) -> Dict[str, bool]:
    ...
    g = {
        ...
        "PASS_CHANNEL_MOMENTUM_PRODUCT_COVARIANCE":
            v("channel_momentum_geoflow", "product_gauge_p99_mean", 1.0) < 1e-7,

        "PASS_CHANNEL_ADAPTIVE_PRODUCT_COVARIANCE":
            v("channel_adaptive_geoflow", "product_gauge_p99_mean", 1.0) < 1e-7,

        "PASS_CHANNEL_MOMENTUM_CHANNEL_COVARIANCE":
            max(
                v("channel_momentum_geoflow", "channel_a_gauge_p99_mean", 1.0),
                v("channel_momentum_geoflow", "channel_b_gauge_p99_mean", 1.0),
            ) < 1e-7,

        "PASS_CHANNEL_ADAPTIVE_CHANNEL_COVARIANCE":
            max(
                v("channel_adaptive_geoflow", "channel_a_gauge_p99_mean", 1.0),
                v("channel_adaptive_geoflow", "channel_b_gauge_p99_mean", 1.0),
            ) < 1e-7,
        ...
        "HYPOTHESIS_CHANNEL_MOMENTUM_REDUCES_VARIANCE":
            v("channel_momentum_geoflow", "functional_variance_mean", float("inf"))
            < v("fixed_capacity_split", "functional_variance_mean", -float("inf")),

        "HYPOTHESIS_CHANNEL_ADAPTIVE_REDUCES_VARIANCE":
            v("channel_adaptive_geoflow", "functional_variance_mean", float("inf"))
            < v("channel_momentum_geoflow", "functional_variance_mean", -float("inf")),

        "HYPOTHESIS_CHANNEL_MOMENTUM_IMPROVES_LOSS":
            v("channel_momentum_geoflow", "final_loss_mean", float("inf"))
            < v("fixed_capacity_split", "final_loss_mean", -float("inf")),

        "HYPOTHESIS_CHANNEL_ADAPTIVE_IMPROVES_LOSS":
            v("channel_adaptive_geoflow", "final_loss_mean", float("inf"))
            < v("channel_momentum_geoflow", "final_loss_mean", -float("inf")),

        "HYPOTHESIS_CHANNEL_HISTORY_BEATS_FACTOR_EMA":
            v("channel_momentum_geoflow", "final_loss_mean", float("inf"))
...
def parse_args() -> Config:
    ...
    p.add_argument("--output-dir", default="h1311_results")
    p.add_argument("--no-plots", action="store_true")

    ns, unknown = p.parse_known_args()
    if unknown:
        print(f"[H13.11] ignored notebook/kernel arguments: {unknown}")
...
def main() -> None:
    ...
    print("=" * 120)
    print("H13.11 GAUGE-COVARIANT MOMENTUM AND SECOND-MOMENT GEOFLOW")
    print("=" * 120)
    ...
    payload = {
        "title": "H13.11 gauge-covariant momentum and second-moment GeoFlow",
        "config": asdict(cfg),
        ...
        "interpretation": {
            ...
            "channel_adaptive": "Channel first moment normalized by channel scalar second moments.",
    ...
    print("\nDecision guide:")
    print("  1. PASS_CORE => channel-history state preserves exact gauge covariance.")
    print("  2. Channel momentum lower VF and lower loss => history helps without coordinate moments.")
    print("  3. Channel adaptive worse than channel momentum => scalar second moments are a negative result.")
    print("  4. Factor EMA vs channel momentum separates transported factor history from intrinsic channel history.")
    print("  5. AdamW may retain strong task performance while remaining gauge dependent.")
