...
# -*- coding: utf-8 -*-
"""
H13.13C — ATTENTION-ONLY TRANSFORMER / LoRA ABLATION
...
self-contained Transformer/LoRA mechanism audit extending H13.13B.

Formal attention-only ablation
------------------------------
python h1313c_attention_only_ablation.py \
  --trials 6 --steps 200 --train-samples 1024 --val-samples 256 \
  --batch-size 16 --probe-steps 0,50,100,150,199 \
  --target-scope attention_only --output-dir h1313c_attention_only_results
...
class Config:
    ...
    target_scope: str = "attention_only"  # fixed default for this ablation
    ...
    dtype: str = "float64"
    device: str = "cpu"
    output_dir: str = "h1313c_attention_only_results"
    no_plots: bool = False
...
def parse_args() -> Config:
    ...
    while i < len(unknown):
        if unknown[i] == "-f" and i + 1 < len(unknown):
            ...
            i += 1
    if ignored:
        print(f"[H13.13C] ignored notebook/kernel arguments: {ignored}")
    return Config(**vars(args))


def main() -> int:
    cfg = parse_args()
    if cfg.target_scope != "attention_only":
        print(
            f"[H13.13C] overriding target_scope={cfg.target_scope!r} "
            "to 'attention_only' for the dedicated ablation."
        )
        cfg.target_scope = "attention_only"
    out_dir = Path(cfg.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 120)
    print("H13.13C ATTENTION-ONLY TRANSFORMER / LoRA ABLATION")
    print("=" * 120)
    ...
    payload = {
        "title": "H13.13C attention-only Transformer/LoRA ablation",
        "config": asdict(cfg),
        "summary": summary,
        "paired_comparisons": {
