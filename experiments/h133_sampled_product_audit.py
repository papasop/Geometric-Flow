# ============================================================
# GeoFlow H12.7F — Actuator-aware K=1 deviation-restoration controller
#
# One file only. It:
...
#       --seeds 1801,2003,2203,2411,2609,2801,3001,3203,3407,3607 \
#       --backward-budgets 300 \
...
# =====================================================================
# H13.3 — PRODUCT-SPACE GAUGE-DYNAMICS AUDIT
# =====================================================================

def _h133_parse_ints(raw: str) -> list[int]:
    values = [int(x.strip()) for x in raw.split(",") if x.strip()]
    if not values:
...
def _h133_parse_floats(raw: str) -> list[float]:
    values = [float(x.strip()) for x in raw.split(",") if x.strip()]
    if not values:
...
def _h133_make_sample_plan(
    ...
    *,
    ...
    """
    Fixed coordinate probes of B@A.

    The probes are identical across balanced and gauge-equivalent branches.
    Sampling coordinates rather than serializing full products keeps H13.3
    lightweight while preserving an unbiased empirical view of product-space
    magnitude, direction, and trajectory separation.
    """
    ...
    for adapter in adapters:
        out_features = int(adapter.B.shape[0])
        in_features = int(adapter.A.shape[1])
        n_total = out_features * in_features
        n = min(int(samples_per_adapter), n_total)

        # Sample flattened product coordinates without replacement.
        flat = torch.randperm(
            n_total,
            generator=generator,
            dtype=torch.int64,
        )[:n]
        rows = torch.div(flat, in_features, rounding_mode="floor")
        cols = flat.remainder(in_features)
...
@torch.no_grad()
def _h133_product_sketch(
    adapters: list[LoRAConv1DAdapter],
    plans: list[tuple[torch.Tensor, torch.Tensor]],
) -> torch.Tensor:
    pieces: list[torch.Tensor] = []
    for adapter, (rows_cpu, cols_cpu) in zip(adapters, plans):
        rows = rows_cpu.to(adapter.B.device)
        cols = cols_cpu.to(adapter.A.device)

        # (B@A)[i,j] = sum_r B[i,r] A[r,j]
        values = (
            adapter.B.index_select(0, rows)
            * adapter.A.index_select(1, cols).T
        ).sum(dim=1)
        pieces.append(values.detach().float().cpu())

    return torch.cat(pieces, dim=0)


def _h133_cosine(x: torch.Tensor, y: torch.Tensor) -> float:
...
def _h133_run_branch(
    *,
    ...
    optimizer, stepper = _h133_make_optimizer(
        optimizer_name=optimizer_name,
        adapters=adapters,
        adamw_lr=adamw_lr,
    ...
    plans = _h133_make_sample_plan(
        adapters,
        samples_per_adapter=samples_per_adapter,
        seed=probe_seed,
    )
    initial_product = _h133_product_sketch(adapters, plans)
    ...
    for step_index in range(steps):
        ...
        product_after = _h133_product_sketch(adapters, plans)
        ...
        rows.append(
            {
                ...
                "sampled_product_norm": float(
                    torch.linalg.vector_norm(product_after)
                ),
                "sampled_delta_product_norm": float(
                    torch.linalg.vector_norm(delta_product)
                ),
                "sampled_displacement_from_initial": float(
...
def _h133_pair_metrics(
    ...
    initial_gap = _h133_relative_gap(
        ...
        out.append(
            {
                ...
                "trajectory_relative_gap": _h133_relative_gap(pb, pg),
    ...
    for optimizer_name in optimizers:
        for kappa in kappas:
            ...
            for source_field, output_field in aggregate_fields.items():
                ...
                ci_low, ci_high = _h133_bootstrap_mean_ci(
                    values,
                    seed=(
                        133000
                        + int(kappa)
...
def h133_main() -> None:
    ...
    parser.add_argument(
        "--repo-dir",
        default="/content/Geometric-Flow-h133-product-audit",
    )
    parser.add_argument("--force-reclone", action="store_true")
    parser.add_argument("--model-name", default="gpt2")
    parser.add_argument(
    ...
    parser.add_argument("--samples-per-adapter", type=int, default=8192)
    parser.add_argument("--data-seed", type=int, default=424242)
    parser.add_argument("--sequence-length", type=int, default=96)
    parser.add_argument("--batch-size", type=int, default=2)
    parser.add_argument("--train-examples", type=int, default=8000)
    ...
    parser.add_argument(
        "--out-dir",
        default="/content/geoflow_h133_product_audit_results",
    )
    args, unknown = parser.parse_known_args()
    if unknown:
        print("[H13.3] ignored notebook/kernel arguments:", unknown)

    seeds = _h133_parse_ints(args.seeds)
    kappas = _h133_parse_floats(args.kappas)
    if any(k <= 1.0 for k in kappas):
        raise ValueError("H13.3 kappas must be > 1.")
    if args.steps < 1:
        raise ValueError("--steps must be positive.")
    if args.samples_per_adapter < 128:
        raise ValueError("--samples-per-adapter should be >= 128.")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("=" * 120)
    print("H13.3 FINAL-v2 PRODUCT-SPACE GAUGE-DYNAMICS AUDIT")
    print("=" * 120)
    print("device =", device)
    print("seeds =", seeds)
    print("kappas =", kappas)
    print("steps =", args.steps)
    print("samples_per_adapter =", args.samples_per_adapter)
    ...
    for seed in seeds:
        ...
        for optimizer_name in ["adamw", "capacity"]:
            ...
            for kappa in kappas:
                ...
                rows = _h133_pair_metrics(balanced, gauge)
    ...
    summaries = _h133_summarize(pair_rows, steps=args.steps)
    contrasts = _h133_optimizer_contrast(summaries)
    ...
    _h132_write_csv(
        out_dir / "h133_stepwise_product_metrics.csv",
        clean_pair_rows,
    )
    _h132_write_csv(
        out_dir / "h133_summary.csv",
        summaries,
    )
    _h132_write_csv(
        out_dir / "h133_optimizer_contrast.csv",
        contrasts,
    )
    _h132_write_csv(
        out_dir / "h133_branch_conditions.csv",
        branch_metadata,
    )

    (out_dir / "h133_stepwise_product_metrics.json").write_text(
        json.dumps(clean_pair_rows, indent=2),
        encoding="utf-8",
    )
    (out_dir / "h133_summary.json").write_text(
        json.dumps(summaries, indent=2),
        encoding="utf-8",
    )
    (out_dir / "h133_optimizer_contrast.json").write_text(
        json.dumps(contrasts, indent=2),
        encoding="utf-8",
    )
    (out_dir / "h133_config.json").write_text(
        json.dumps(
            {
                "experiment": "H13.3",
                "role": "product-space gauge-dynamics mechanism audit",
                "model_name": args.model_name,
                "dataset_config": args.dataset_config,
                "target_modules": target_modules,
                "rank": args.rank,
                ...
                "prediction": (
                    "AdamW errors grow with gauge conditioning while "
                    "Capacity remains approximately product-space equivariant."
                ),
                "important_note": (
                    "Product metrics use fixed sampled coordinates of B@A. "
