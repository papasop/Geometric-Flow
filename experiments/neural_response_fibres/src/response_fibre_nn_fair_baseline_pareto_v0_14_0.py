#!/usr/bin/env python3
"""Fair-baseline and Pareto qualification audit v0.14.0.

This development-only residual-CNN experiment repairs the structural defect of
the fixed-step soft-penalty baselines used in v0.13.0.  At an exactly matched
initial response, the squared-penalty gradient vanishes, so the first penalty
step is identical to unconstrained descent and may immediately leave the
declared response budget.  Here every non-geometric method uses a declared-
response backtracking line search; augmented-Lagrangian baselines are added.
...
The default seeds are the already exposed v0.13.0 seeds.  Consequently this
script can qualify a baseline architecture for a later prospective v0.14.1,
but it cannot support a new neural task-advantage claim by itself.
"""
...
DEVELOPMENT_SEEDS = (20270307, 20270319, 20270403)
...
class Gates:
    ...
    minimum_qualified_seed_fraction: float = 2.0 / 3.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", default="response_fibre_nn_fair_baseline_pareto_v0_14_0_results"
    )
    parser.add_argument(
        "--seeds",
        default=",".join(str(value) for value in DEVELOPMENT_SEEDS),
        help="development seeds only; overrides remain non-claim diagnostics",
    )
...
def main() -> int:
    ...
    gates = Gates()
    protocol = {
        "title": "RESPONSE-FIBRE FAIR NON-GEOMETRIC BASELINE / PARETO QUALIFICATION AUDIT",
        "version": VERSION,
        "formal_interval_arithmetic": False,
        "purpose": "qualify declared-response-backtracking and augmented-Lagrangian non-geometric comparators before a new prospective task-advantage audit",
        ...
        "development_seeds_reused_from_v0_13": list(DEVELOPMENT_SEEDS),
        "new_task_advantage_claim_permitted": False,
        "gates": asdict(gates),
    ...
    print("=" * 112)
    print(f"RESPONSE-FIBRE FAIR BASELINE / PARETO QUALIFICATION AUDIT v{VERSION}")
    print("=" * 112)
    ...
    for index, seed in enumerate(seeds, 1):
        ...
        print(
            f"  both_levels_qualified="
            f"{result['both_response_levels_baseline_qualified']}",
            flush=True,
        )
        for level in LEVELS:
            item = result["levels"][level]
            print(
                f"    {level}: qualified={item['baseline_qualification_pass']} "
    ...
    passing = sum(
        bool(item["both_response_levels_baseline_qualified"])
        for item in seed_results
    )
    fraction = passing / len(seed_results)
    qualification_gate = fraction >= gates.minimum_qualified_seed_fraction
    status = (
        "FAIR_NON_GEOMETRIC_BASELINE_ARCHITECTURE_QUALIFIED_FOR_PROSPECTIVE_AUDIT"
        if qualification_gate
        else "FAIR_NON_GEOMETRIC_BASELINE_ARCHITECTURE_NOT_YET_QUALIFIED"
    )
    report = {
        "scientific_status": status,
        "all_baseline_qualification_gates_pass": qualification_gate,
        "formal_interval_arithmetic": False,
        "development_only": True,
        "new_task_advantage_claimed": False,
        "protocol_sha256": protocol_hash,
        "seeds_declared": len(seeds),
        "seeds_qualifying_both_response_levels": passing,
        "qualified_seed_fraction": fraction,
        "qualification_cohort_gate_pass": qualification_gate,
        ...
        "next_required_step": (
            "Freeze this exact baseline architecture and gates, then assign entirely new prospective seeds in v0.14.1."
            if qualification_gate
            else "Inspect which baseline stalls or violates held-out/task gates; repair the baseline architecture before assigning new seeds."
        ),
        "scope": "development-only small synthetic residual-CNN baseline qualification; not a theorem, task-advantage claim, real-data result, or large-model result",
    }
    ...
    if qualification_gate:
        print("\nPASS: fair non-geometric comparators now exist; freeze them before v0.14.1.")
    else:
        print("\nINCONCLUSIVE: the comparator architecture is not yet qualified; do not assign prospective seeds.")
    return 0
