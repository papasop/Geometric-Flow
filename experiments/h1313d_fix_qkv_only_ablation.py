...
# -*- coding: utf-8 -*-
"""
H13.13D-FIX — QKV-ONLY TRANSFORMER / LoRA ABLATION
...
self-contained Transformer/LoRA mechanism audit extending H13.13C.

Formal run
----------
python h1313d_fix_qkv_only_ablation.py \
  --trials 6 --steps 200 --train-samples 1024 --val-samples 256 \
  --batch-size 16 --probe-steps 0,50,100,150,199 \
  --target-scope qkv_only --output-dir h1313d_qkv_only_results
...
class Config:
    ...
    target_scope: str = "qkv_only"  # fixed by dedicated ablation
    ...
    dtype: str = "float64"
    device: str = "cpu"
    output_dir: str = "h1313d_qkv_only_results"
    no_plots: bool = False
...
def train_method(
    ...
    row: Dict[str, object] = {
        ...
        "qkv_scope_exact": bool(list(model.lora_layers().keys()) == [f"blocks.{i}.qkv" for i in range(cfg.n_layers)]),
...
def build_gates(
    ...
    gates: Dict[str, object] = {
        ...
        "PASS_QKV_SCOPE_EXACT": all(bool(r["qkv_scope_exact"]) for r in rows),
...
def parse_args() -> Config:
    ...
    while i < len(unknown):
        if unknown[i] == "-f" and i + 1 < len(unknown):
            ...
            i += 1
    if ignored:
        print(f"[H13.13D-FIX] ignored notebook/kernel arguments: {ignored}")
    return Config(**vars(args))


def main() -> int:
    cfg = parse_args()
    if cfg.target_scope != "qkv_only":
        print(
            f"[H13.13D-FIX] overriding target_scope={cfg.target_scope!r} "
            "to 'qkv_only' for the dedicated ablation."
        )
        cfg.target_scope = "qkv_only"
    out_dir = Path(cfg.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 120)
    print("H13.13D-FIX QKV-ONLY TRANSFORMER / LoRA ABLATION")
    print("=" * 120)
    ...
    for trial in range(cfg.trials):
        ...
        expected_layers = [f"blocks.{i}.qkv" for i in range(cfg.n_layers)]
        ...
        if active_layers != expected_layers:
            raise RuntimeError(
                "qkv_only scope mismatch: "
                f"expected {expected_layers}, got {active_layers}"
            )
        for layer_name in active_layers:
            if not layer_name.endswith(".qkv"):
                raise RuntimeError(f"non-QKV LoRA layer active: {layer_name}")
    ...
    payload = {
        "title": "H13.13D-FIX qkv-only Transformer/LoRA ablation",
        "config": asdict(cfg),
        "summary": summary,
        "paired_comparisons": {
    ...
    print("\n" + "=" * 120)
    print("H13.13D-FIX METHOD SUMMARY")
    print("=" * 120)
    for row in summary:
        print(
            f"{row['method']:30s} "
